# MS-Client-UI Project

ms-client-ui is a microservice in the Java application Medilabo, built with Maven and deployed using Docker. This guide will help you build, test, and deploy the Docker image.
...
```bash
docker build -t your-dockerhub-username/ms-client-ui:latest .
```
...
```bash
docker run -d -p 8080:8080 your-dockerhub-username/ms-client-ui:latest
```
...
```bash
docker push your-dockerhub-username/ms-client-ui:latest
```
...
You have successfully built, tested, and deployed the Docker image for the MS-Client-UI project. This process can be integrated into your CI/CD pipeline using the provided GitHub Actions workflow.