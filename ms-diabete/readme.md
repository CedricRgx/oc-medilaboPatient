...
ms-diabete is a microservice in the Java application Medilabo, built with Maven and deployed using Docker. This guide will help you build, test, and deploy the Docker image.
...
```bash
docker build -t your-dockerhub-username/ms-diabete:latest .
```
...
```bash
docker run -d -p 8084:8084 your-dockerhub-username/ms-diabete:latest
```

This command will start the container and make the application accessible on port `8084`.
...
```bash
docker push your-dockerhub-username/ms-diabete:latest
```
...
You have successfully built, tested, and deployed the Docker image for the MS-Diabete project. This process can be integrated into your CI/CD pipeline using the provided GitHub Actions workflow.