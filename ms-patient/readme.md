...
MS-Patient is a microservice in the Java application Medilabo, built with Maven and deployed using Docker. This guide will help you build, test, and deploy the Docker image.
...
```bash
docker run -d -p 8081:8081 your-dockerhub-username/ms-diabete:latest
```

This command will start the container and make the application accessible on port `8081`.
...
```bash
docker push your-dockerhub-username/ms-note:latest
```
...
You have successfully built, tested, and deployed the Docker image for the MS-Patient project. This process can be integrated into your CI/CD pipeline using the provided GitHub Actions workflow.