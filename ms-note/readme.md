...
ms-note is a microservice in the Java application Medilabo, built with Maven and deployed using Docker. This guide will help you build, test, and deploy the Docker image.
...
```bash
docker build -t your-dockerhub-username/ms-note:latest .
```
...
```bash
docker run -d -p 8083:8083 your-dockerhub-username/ms-note:latest
```

This command will start the container and make the application accessible on port `8083`.
...
You have successfully built, tested, and deployed the Docker image for the MS-Note project. This process can be integrated into your CI/CD pipeline using the provided GitHub Actions workflow.