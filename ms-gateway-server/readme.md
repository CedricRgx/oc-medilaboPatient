# MS-Gateway-server Project

MS-Gateway-server is a microservice in the Java application Medilabo, built with Maven and deployed using Docker. This guide will help you build and deploy the Docker image.
...
```bash
docker build -t your-dockerhub-username/ms-gateway-server:latest .
```
...
```bash
docker run -d -p 8082:8082 your-dockerhub-username/ms-gateway-server:latest
```

This command will start the container and make the application accessible on port `8082`.
...
```bash
docker push your-dockerhub-username/ms-gateway-server:latest
```
